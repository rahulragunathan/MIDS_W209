var sankey_margin = {
  ...
  sankey_height = 600 - sankey_margin.top - sankey_margin.bottom;
...
d3.json("data/county.json", function(water_flow) {
  ...
  sankey
	.nodes(water_flow[county_selection].nodes)
	.links(water_flow[county_selection].links)
	.layout(32);
	
	var total_usage = water_flow[county_selection].total_usage

  var link = svg.append("g").selectAll(".link")
	.data(water_flow[county_selection].links)
	.enter().append("path")
	.attr("class", "link")
	.attr("d", path)
  ...
  var node = svg.append("g").selectAll(".node")
	.data(water_flow[county_selection].nodes)
	.enter().append("g")
	.attr("class", "node")
